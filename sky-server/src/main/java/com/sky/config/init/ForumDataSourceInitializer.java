...
public class ForumDataSourceInitializer {
    ...
    private boolean autoInitDatabase() {
        ...
        URI url = URI.create(Objects.requireNonNull(environment.getProperty("spring.datasource.druid.url")).substring(5));
        String uname = environment.getProperty("spring.datasource.druid.username");
        String pwd = environment.getProperty("spring.datasource.druid.password");
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://" + url.getHost() + ":" + url.getPort() +
                "?serverTimezone=Asia/Shanghai&useUnicode=true&characterEncoding=utf-8&zeroDateTimeBehavior=convertToNull&useSSL=false&allowPublicKeyRetrieval=true", uname, pwd);
